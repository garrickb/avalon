defmodule Avalon.Quest do
  ...
  Creates a new quest.
  """
  def new(num_players, num_fails_required) when is_number(num_players) and is_number(num_fails_required) do
    %Quest{num_players: num_players,
  ...
  @doc """
  Marks a player as selected. NoOp if they are already selected.
  """
  def select_player(quest, player_name) when is_binary(player_name) do
      case quest.outcome do
        {:uncompleted} ->
        ...
        _ ->
          Logger.warn("Attempted to select player '#{player_name}' in already completed quest: #{inspect(quest)}")
          quest
      end
  end

  @doc """
  Marks a player as unselected. NoOp if they are already unselected.
  """
  def deselect_player(quest, player_name) when is_binary(player_name) do
    case quest.outcome do
      {:uncompleted} ->
        if Enum.member?(quest.selected_players, player_name) do
          %{quest | selected_players: quest.selected_players |> Enum.filter(fn n -> n != player_name end)}
        else
          quest
        end

      _ ->
        Logger.warn("Attempted to deselect player '#{player_name}' in already completed quest: #{inspect(quest)}")
        quest
    end
  end
